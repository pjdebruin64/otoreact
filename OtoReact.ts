...
function DefConstr(c: ConstructDef) {
    /*
    let C = env.cdefs, prevDef = C.get(c.nm);
    envActions.push(() => mapSet(C,c.nm, prevDef));
    mapNm(C, c);
    /*/
    let C = env.cdefs;
    if (C.has(c.nm)) {
...
class RCompiler {
    ...
    private CSignats: Map<string, Signature>;
    ...
    private AddConstructs(listS: Iterable<Signature>) {
        for (let S of listS) {
            ...
            this.restoreActions.push(() => 
                mapSet(this.CSignats, S.nm, savedC)
            );
        }
    }
    ...
    private bCompiled: boolean;

    private wspc = WSpc.block;  // While compiling: whitespace mode for the node(s) to be compiled; see enum WSpc
    private rspc: bool = T;     // While compiling: may the generated DOM output be right-trimmed
    ...
    private async CompElm(srcPrnt: ParentNode, srcElm: HTMLElement, bUnhide?: boolean
        ...
        let // List of source attributes, to check for unrecognized attributes
            ...
            anyH: bool,     // Truthy when there is any before or after event hanldeer
                        ...
                        try {
                            ...
                            while(T) {
                                ...
                                if (hash == N
                                    ...
                                    {
                                        this.RVAR_Light<Item>(item, getUpdatesTo && [getUpdatesTo()]);
                                        if (childRange.subs)
...
export let 
    R = new RCompiler(),
    RVAR = globalThis.RVAR as <T>(name?: string, initialValue?: T|Promise<T>, store?: Store, subs?: Subscriber<T>, storeName?: string) => RVAR<T>, 
...
Object.assign(
    globalThis, {range: _rng, reroute, RFetch}
)
