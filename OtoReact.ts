...
class Range<NodeType extends ChildNode = ChildNode> {
    ...
    next: Range;     // Next item in linked list
    parentR?: Range;
    newParent?: Node;
...
type LVar = ((value?: unknown, bUpd?: bool) => void) & {nm: string};
...
function ApplyMod(elm: HTMLElement, M: Modifier, val: unknown, bCreate: boolean) {
    let {mt, nm, cnm} = M;
    function checkNm() {
        if (!cnm)
...
class RCompiler {
        ...
        try {
            ...
            if (constr)
                ...
                switch (srcElm.localName) {
                    ...
                    case 'rhtml': {
                        let getSrctext = this.CompParam<string>(atts, 'srctext', 1)
                    ...
                    case 'document': {
                        ...
                        bldr = async function DOCUMENT(this: RCompiler, area: Area) {
                            ...
                            if (bInit) {
                                ...
                                rng.value = {
                                    async render(W: Window, bInit: bool, args: unknown[]) {
    ...
    private ParseSignat(elmSignat: Element, bIsSlot?: bool):  Signature {
    ...
    private CompParam<T = unknown>(atts: Atts, attName: string, bReq?: bool): Dependent<T> {
    ...
    async fetchModule(src: string): Promise<Iterable<ChildNode>> {
        ...
        if (!mod) {
            ...
            if (mod.firstElementChild?.tagName != 'MODULE')
                return concIter(doc.head.childNodes, mod.childNodes);
...
function CapitalProp(nm: string) {
    ...
    return nm.replace(regCapit, (w, w1) => {
        let r = b ? w.substring(0,1).toUpperCase() + w.substring(1) : w;
        b = w1;
        return r;
    });
}
