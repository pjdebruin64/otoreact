...
if(!node)
this.child=null
if(area&&!((_a=area.parentR)==null?void 0:_a.node))
this.parentR=area.parentR
}
toString(){var _a;return this.text||((_a=this.node)==null?void 0:_a.nodeName)}
get First(){
let f
if(f=this.node)
return f
...
result&&(result=(_a=sig.Slots.get(slotname))==null?void 0:_a.IsCompatible(slotSig))
return !!result
}
}
const gEval=eval
...
this.context=(RC==null?void 0:RC.context)||""
...
const subs=this.Subscriber(area,this.Builder,(parentR==null?void 0:parentR.child)||area.prevR)
...
RC.AddDirty(sub)
if((_a=t._UpdatesTo)==null?void 0:_a.length)
...
const rvarName=atts.get('rvar'),varName=rvarName||atts.get('let')||atts.get('var',true),getValue=this.CompParameter(atts,'value',DUndef),getStore=rvarName&&this.CompAttrExpr(atts,'store'),bReact=CBool((_a=atts.get('reacting'))!=null?_a:atts.get('updating')),newVar=this.NewVar(varName)
if(rvarName){
...
?{regex:new RegExp(p,'i'),lvars:(((_b=atts.get('captures'))==null?void 0:_b.split(','))||[]).map(this.NewVar.bind(this))
}
:null
if(bHiding&&(patt==null?void 0:patt.lvars.length))
...
case'document':
{
const newVar=this.NewVar(atts.get('name',true)),bEncaps=CBool(atts.get('encapsulate')),params=atts.get('params'),RC=this,saved=this.SaveContext(),setVars=((params==null?void 0:params.split(','))||[]).map(v=>this.NewVar(v))
try{
...
g.handler().call((_a=(range||area.prevR))==null?void 0:_a.node)
}
}
...
throw message
console.log(message)
if(onerror==null?void 0:onerror.bBldr)
onerror(err)
else if(this.Settings.bShowErrors){
const errorNode=area.parent.insertBefore(createErrorNode(message),(_a=area.range)==null?void 0:_a.FirstOrNext)
if(range)
...
var _a
const bMod=((_a=atts.get('type'))==null?void 0:_a.toLowerCase())=='module',bNoMod=atts.get('nomodule')!=null,defines=atts.get('defines')
...
var _a,_b
let varName=(_a=atts.get('let'))!=null?_a:atts.get('var'),ixName=atts.get('index'),saved=this.SaveContext()
if(ixName=='')
ixName='index'
try{
if(varName !=null){
...
if(nextName=='')
nextName='next'
const getRange=this.CompAttrExpr(atts,'of',true),getUpdatesTo=this.CompAttrExpr(atts,'updates'),bReacting=CBool((_b=atts.get('reacting'))!=null?_b:atts.get('reactive'))||!!getUpdatesTo,initVar=this.NewVar(varName),initIndex=this.NewVar(ixName),initPrev=this.NewVar(prevName),initNext=this.NewVar(nextName),getKey=this.CompAttrExpr(atts,'key'),getHash=this.CompAttrExpr(atts,'hash'),bodyBuilder=await this.CompChildNodes(srcElm)
...
setVar(item)
setIndex(idx)
const hash=getHash&&getHash(),key=(_a=getKey==null?void 0:getKey())!=null?_a:hash
if(key !=null&&newMap.has(key))
throw`Key '${key}' is not unique`
newMap.set(key !=null?key:{},{item,hash,idx})
idx++
}
}
...
if(nextIterator)
nextItem=(_b=nextIterator.next().value)==null?void 0:_b.item
let childRange=keyMap.get(key),bInit=!childRange
if(bInit){
subArea.range=null
subArea.prevR=prevRange
subArea.before=(nextChild==null?void 0:nextChild.FirstOrNext)||before
;
...
if(childRange.fragm){
const nextNode=(nextChild==null?void 0:nextChild.FirstOrNext)||before
...
const nextIndex=(_c=newMap.get(nextChild.key))==null?void 0:_c.idx
if(nextIndex>idx+2){
...
if(arg===undefined&&(dflt=(_a=signat.Params[i])==null?void 0:_a.pDefault))
arg=dflt()
lvar()(arg)
i++
}
...
if(signat.RestParam)
ApplyModifier(elm,8,null,args[signat.RestParam.name],bInit)
...
if(/^on/.test(name))
modifs.push({mType:5,name,depV:this.AddErrHandler(depV)})
else
...
result+=typeof gen=='string'?gen:(_a=gen.call(this))!=null?_a:''
return result
}
catch(err){
...
let result=""
for(const gen of generators)
result+=typeof gen=='string'?gen:(_a=gen())!=null?_a:''
return result
}
catch(err){
throw name?`[${name}]: ${err}`:err
...
if(!r.ok)
throw`${(init == null ? void 0 : init.method) || 'GET'} ${input} returned ${r.status} ${r.statusText}`
return r
}
...
if(sub.bImm)
sub(this._Value)
else if(!((_b=(_a=sub.sArea)==null?void 0:_a.range)==null?void 0:_b.erased)){
...
if(location.hash)
setTimeout((()=>{var _a;return(_a=document.getElementById(location.hash.substring(1)))==null?void 0:_a.scrollIntoView()}),6)
}
